...
module.exports = async (parm) => {
  ...
  if (type === 'quote') {
    ...
    // Watermark removed for clean sticker
    ...
    // Direct bubble buffer, no extra shadow/padding for stickers
  ...
  if (ext) {
    if (ext === 'webp') {
      image = await sharp(quoteImage).webp({ lossless: true, force: true }).toBuffer()
    } else if (ext === 'png') {
      image = await sharp(quoteImage).png({ lossless: true, force: true }).toBuffer()
    } else {
      image = quoteImage
    }
  } else {
